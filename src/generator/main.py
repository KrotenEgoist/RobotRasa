...
direction = g.generate_direction(amount=20)
g.save({"directions": direction}, nlu_path)

action_direction = g.generate_action_direction(amount=15)
...
action_objects = g.generate_action_object(states=20, amount=10)
g.save({"action+objects": action_objects}, nlu_path)

patrol = g.generate_patrol(amount=25)
